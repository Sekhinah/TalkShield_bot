# app_webhook.py
# Talk_Shield — webhook version for Render
# - Uses python-telegram-bot built-in aiohttp webhook server
# - Routes EN -> toxicity (multi-label), AK -> sentiment (3-class)
# - In groups: deletes if any harmful score >= threshold
# - In DMs: replies with full report
...
# Config
# ---------------------------------------------------------
ENG_MODEL_ID = os.getenv("ENG_MODEL_ID", "Sekhinah/Talk_Shield_English")  # 7-label toxicity
TWI_MODEL_ID = os.getenv("TWI_MODEL_ID", "Sekhinah/Talk_Shield")          # 3-class sentiment
...
def route_and_classify_structured(
    ...
    Returns structured dict:
      {'lang','model_id','probs','picked','pred','report'}
    """
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Commands:\n"
        f"/threshold 0.6 — set toxicity threshold (now {thr:.2f})\n"
        f"/lang ak|en|auto — force language (now {lang_pref.upper()})\n"
...
def main():
    ...
    # Render gives you PORT & sometimes RENDER_EXTERNAL_URL
    port = int(os.getenv("PORT", "8000"))
    webhook_path = os.getenv("WEBHOOK_PATH", token)  # secret-ish path; default = token
    base_url = (os.getenv("BASE_URL", "") or os.getenv("RENDER_EXTERNAL_URL", "")).rstrip("/")
    if not base_url:
        # First deploy: you can fill BASE_URL after Render assigns it.
        log.warning("BASE_URL/RENDER_EXTERNAL_URL is empty. Set it after first deploy, then redeploy.")
    ...
    application.add_handler(MessageHandler(~filters.TEXT & ~filters.COMMAND, handle_nontext))

    log.info("Starting webhook server…")
    # This will (re)register the webhook automatically if webhook_url is given
    application.run_webhook(
        listen="0.0.0.0",
        port=port,
        url_path=webhook_path,
        webhook_url=(f"{base_url}/{webhook_path}" if base_url else None),
        stop_signals=None,  # Render sends SIGTERM; PTB handles clean shutdown
    )
